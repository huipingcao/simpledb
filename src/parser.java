...
public class parser {
            ...
            if (isJoin) {       //join node
                ...
                if (op == Predicate.Op.EQUALS) {
                    try {
                        ...
                        //System.out.println("Loading join: " + c);
    ...
    public static void loadSchema(String catalogFile) {
        String line = "";
        try {
            ...
            while ((line = br.readLine()) != null) {
                ...
                HeapFile tabHf = new HeapFile(new File(name + ".dat"));
                Database.getCatalog().addTable(tabHf,t,name);
    ...
    public static void handleQueryStatement(ZQuery s) throws TransactionAbortedException, DbException, IOException, ParsingException, Zql.ParseException  {
        // and run it
        DbIterator node;
        node = parseQuery(curtrans.tid(), s);

        Query sdbq = new Query(node, curtrans.tid());
    ...
    public static void handleInsertStatement(ZInsert s) throws TransactionAbortedException, DbException, IOException, ParsingException, Zql.ParseException  {
        ...
        if (s.getValues() != null) {
            ...
            newTups = parseQuery(curtrans.tid(),query);
        }

        Query sdbq = new Query(new Insert(curtrans.tid(), newTups, id), curtrans.tid());
        // XXX print field names
        sdbq.start();
    ...
    public static void handleDeleteStatement(ZDelete s) throws TransactionAbortedException, DbException, IOException, ParsingException, Zql.ParseException  {
        ...
        //replace the TupleDesc in it with the same tuple desc where every field name is prefaced by the table alias
        DbIterator it = new SeqScan(curtrans.tid(),id,name);
        tableMap.put(name, it);
        System.out.println("     TABLE HAS  tupleDesc " + it.getTupleDesc());

        if (s.getWhere() != null)
            processExpression( curtrans.tid(), (ZExpression)s.getWhere(), tableMap, new HashMap<String,String>());

        Query sdbq = new Query(new Delete(curtrans.tid(), tableMap.get(name)), curtrans.tid());
        // XXX print field names
        sdbq.start();
    ...
    public static void main(String argv[]) throws IOException {
        ...
        if (argv.length == 2) {
        	...
        	System.out.println("Enter a SQL statement or press CTRL-D to quit");
            ...
            while ((line = reader.readLine("SimpleDB> ")) != null) {
                ...
                while (line.indexOf(';') >= 0) {
                    ...
                    if (curtrans == null) {
                        curtrans = new Transaction();
                        curtrans.start();
                        System.out.println("Started a new transaction tid = " + curtrans.tid().getId());
                    }
        ...
        // Since we do not implement transactions, we force all pages to disk
        // to make the changes permanent before quitting
